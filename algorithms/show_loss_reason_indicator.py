...
def analyse(farmName, typeName:list, startTime, endTime):
    #赋值
    fault_loss_all, turbine_FaultLoss_dict = selectFaultLossAll(pd.DataFrame(), farmName, typeName, datetime.strptime(startTime), datetime.strptime(endTime))
    limturbine_loss_all, turbine_LimturbineLoss_dict = selectLimturbineLossAll(pd.DataFrame(), farmName, typeName, datetime.strptime(startTime), datetime.strptime(endTime))
    stop_loss_all, turbine_StopLoss_dict = selectStopLossAll(pd.DataFrame(), farmName, typeName, datetime.strptime(startTime), datetime.strptime(endTime))
    limgrid_loss_all, turbine_LimgridLoss_dict = selectLimgridLossAll(pd.DataFrame(), farmName, typeName, datetime.strptime(startTime), datetime.strptime(endTime))
    faultgrid_loss_all, turbine_FaultgridLoss_dict = selectFaultgridLossAll(pd.DataFrame(), farmName, typeName, datetime.strptime(startTime), datetime.strptime(endTime))
    Technology_loss_all, turbine_TechnologyLoss_dict = selectTechnologyLossAll(pd.DataFrame(), farmName, typeName, datetime.strptime(startTime), datetime.strptime(endTime))
    eny_wspd_all, turbine_EnyWspd_dict = selectEnyWspdAll(pd.DataFrame(), farmName, typeName, datetime)
   ...
   #################################
    ...
    #平均风速
    result['indicator']['平均风速'] = np.nansum(eny_wspd_temp['wspd'].multiply(eny_wspd_temp['count']))/np.nansum(eny_wspd_temp['count'])
    #实发电量
    result['indicator']['实发电量'] = np.nansum(eny_wspd_temp['eny'])
    #等效小时
    result['indicator']['等效小时'] = np.nansum(eny_wspd_temp['eny'])/np.nansum(eny_wspd_temp.loc[~eny_wspd_temp['wtid'].duplicated(),'Rate_power'])
    ...
    result['reason']['机组故障'] = np.nansum(fault_loss_show['loss'])
    result['reason']['电网限电'] = np.nansum(limgrid_loss_show['loss'])
    result['reason']['电网故障'] = np.nansum(faultgrid_loss_show['loss'])
    result['reason']['机组限电'] = np.nansum(limturbine_loss_show['loss'])
    result['reason']['技术待机'] = np.nansum(Technology_loss_show['loss'])
    result['reason']['计划停机'] = np.nansum(stop_loss_show['loss'])


    #时间可利用率
    result['indicator']['时间可利用率'] = '{:.2%}'. format(turbine_efficiency_function.Time_Avail(startTime,endTime,fault_loss_show))
    #MTBF
    result['indicator']['MTBF'] = '%.1f' %(turbine_efficiency_function.MTBT_Calculate(startTime,endTime,fault_loss_show))  
    #能量可利用率
    result['indicator']['能量可利用率'] = '{:.2%}'. format(turbine_efficiency_function.Eny_Avail(result['indicator']['实发电量'],result['reason']['机组故障'],result['reason']['电网限电'],result['reason']['机组限电'],result['reason']['电网故障'],result['reason']['计划停机'],result['reason']['技术待机']))
    #限电率
    result['indicator']['限电率'] ='{:.2%}'. format(result['reason']['电网限电']/(result['indicator']['实发电量']+result['reason']['电网限电'])) 
    #无故障时长
    result['indicator']['无故障时长'] ='%.0f' %(turbine_efficiency_function.NotFault_Time(startTime,endTime,fault_loss_all_temp,faultgrid_loss_all_temp))
    #故障停机恢复时间
    if (len(fault_loss_show) <= 0)&(len(faultgrid_loss_show) <= 0):
            result['indicator']['故障停机恢复时间'] = 0
    elif (len(fault_loss_show) > 0)&(len(faultgrid_loss_show) <= 0):
        result['indicator']['故障停机恢复时间'] = '%.1f' %(np.sum(fault_loss_show['time']) / np.sum(fault_loss_show['count']))
    elif (len(fault_loss_show) <= 0)&(len(faultgrid_loss_show) > 0):
        result['indicator']['故障停机恢复时间'] = '%.1f' %(np.sum(faultgrid_loss_show['time']) / np.sum(faultgrid_loss_show['count']))
    else:
        result['indicator']['故障停机恢复时间'] = '%.1f' %((np.sum(faultgrid_loss_show['time'])+np.sum(fault_loss_show['time'])) / (np.sum(faultgrid_loss_show['count'])+np.sum(fault_loss_show['count'])))
