...
def analyse(farmName, typeName:list, startTime, endTime):
    #赋值
    limgrid_loss_all, turbine_dict = selectLimgridLossAll(pd.DataFrame(), farmName, typeName, datetime.strptime(startTime, "%Y-%m-%d"), datetime.strptime(endTime, "%Y-%m-%d"))
    ...
    for num in range(len(turbine_list)):
        ...
        if len(temp_turbine) > 0:
            ...
            limgrid_loss_show_temp.loc[num,'wtid'] =  turbine_list[num]
            elem = {}
            #机位号
            elem['wtid'] = '%s'%(limgrid_loss_show_temp.loc[num,'wtid'])
            #故障时长(h)
            elem['faultTime'] = '%.4f'%(limgrid_loss_show_temp.loc[num,'time'])
            #故障损失电量(kwh)
            elem['faultLoss'] = '%.4f'%(limgrid_loss_show_temp.loc[num,'loss'])
            #平均风速(m/s)
            elem['meanWindSpeed'] = '%.4f'%(limgrid_loss_show_temp.loc[num,'wspd'])
